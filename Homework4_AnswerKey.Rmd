...
`paste0` does not insert a space between the 3 character strings, so each character vector `x`, `y`, and `z` run together without a space in between.
...
Write the R code below to merge these 3 strings. _TYPO - this should say merge these 4 strings. Either was graded as correct._
...
Suppose we want to concatenate the 3 strings _(X, Y and Z skipping W)_ we did above, but also NA. We can do this in these two ways. What is the difference? (Answer to yourself)
...
The `paste()` function merges the `X`, `Y` and `Z` character vectors together and puts `NA` on the end of the line. But the `str_c()` only results in a value of `NA`.
...
```{r Answer 5}
mySeparatedString2 <- str_split(myNewString, "_")
mySeparatedString2
```

###Section 6.

On another occasion, I need the same string split so that the last word comes off _(really the 1st word comes off)_, and the rest remains intact. I can achieve that in the following way:
...
HINT: Notice that we asked for the underscore `_` to be used before the dash `-`.
...
HINT: Notice the extra spaces before and after " he " and " she " - you have to add these spaces to get this substitution to work.
...
_FYI - here is what happens is you didn't put the extra spaces for the "he" to "she" substitution. You get wierd words like "sher"" and "shead"" and "tshe" because it keeps substituting "she" for "he" even for partial words._

```{r Answer 10 no space}
punModified <- stri_replace_all_fixed(pun, c("his", "he"), c("her", "she"), vectorize_all = FALSE)
punModified
```